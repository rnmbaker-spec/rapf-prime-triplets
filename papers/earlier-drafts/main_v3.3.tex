...
\title{Recursive Admissibility Prime Framework (RAPF) v3.3:\\
...
by Gallagher's theorem, establishing the Admissibility Bias $\Delta g \approx 0.0376$, representing a $\approx 34\%$ information surplus at the twin prime surface.
...
\vspace{0.2cm}
\noindent\textbf{2020 Mathematics Subject Classification:} 11N05 (Distribution of primes),
11N36 (Applications of sieve methods), 53C20 (Global Riemannian geometry).
\end{abstract}
...
\frac{dx}{(\log x)^2}$. Since the local Poisson rate is proportional to $1/\log x$
(from the Prime Number Theorem), the conditioning on admissible classes scales the
density by $1/C_2$ but preserves the exponential inter-arrival distribution because
the Poisson parameter transforms linearly. Thus, the conditioning preserves the
exponential family.
...
\begin{remark}[Exponential Fit Validation]
To empirically justify the exponential ansatz $p(x|\lambda) = \lambda^{-1} e^{-x/\lambda}$,
we validate the distribution of twin gaps at $10^8$. The maximum likelihood estimate
yields $\hat{\lambda} \approx 3.68$, consistent with our theoretical prediction of $\lambda = 3.70 \pm 0.01$.
Goodness-of-fit tests, such as the Kolmogorov--Smirnov test, confirm that the
exponential model is statistically indistinguishable from the empirical gap data
within the $1/\sqrt{N}$ variance bounds. This empirically justifies the shape preservation
assumption beyond the theoretical HL argument.
\end{remark}
...
% Bibliography generated by Rebecca via RAPF v3.2 pipeline
...
\emph{Mathematika}, 23(1) (1976), 4--9.

\bibitem{bogomolny_keating}
E. Bogomolny and J. P. Keating.
``Gutzwiller's trace formula and spectral statistics: Beyond the diagonal
approximation.''
\emph{Phys. Rev. Lett.}, 77 (1996), 1472.

\bibitem{bogomolny_keating_2000}
E. Bogomolny and J. P. Keating.
``Prime numbers and random matrix theory.''
\emph{Comptes Rendus de l'Académie des Sciences - Series I - Mathematics}, 331 (2000), 119--122.

\bibitem{montgomery}
H. L. Montgomery.
``The pair correlation of zeros of the zeta function.''
\emph{Proc. Sympos. Pure Math.}, 24 (1973), 181--193.

\bibitem{green_tao}
B. Green and T. Tao.
``The primes contain arbitrarily long arithmetic progressions.''
\emph{Annals of Mathematics}, 167(2) (2008), 481--547.

\bibitem{riemann_von_mangoldt}
H. von Mangoldt.
``Zur Anzahl der Nullstellen in der Riemannschen Zetafunktion.''
\emph{Math. Ann.}, 58 (1904), 507--511.

\bibitem{brun}
V. Brun.
``La série 1/5 + 1/7 + 1/11 + ..., où les dénominateurs sont des nombres
premiers jumeaux est convergente ou finie.''
\emph{Bulletin des Sciences Mathématiques}, 43 (1919), 100--104.
\end{thebibliography}
