...
The evaluation complexity for gap $g$ is $O(|A_{n}|^2)$ — quadratic in the admissible
class count, identical to the twin case. The certificate score remains:
\begin{equation}
...
\begin{tabular}{lcccc}
\toprule
$n$ & $P_n$ & Admissible Classes & Expected & Observed \\
\midrule
6 & 30,030 & 4,800 & --- & --- \\
7 & 510,510 & 72,160 & --- & --- \\
8 & 9,699,690 & --- & --- & --- \\
\bottomrule
\end{tabular}
\caption{Sexy prime admissibility census.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Cousin prime admissibility census.}
\end{table}
...
The $k = 3$ case introduces new complexity. The admissible class space becomes 2-dimensional:
the triplet $(p, p+2, p+6)$ requires simultaneous admissibility of all three components.

The polynomial certificate complexity jumps from $O(|A|^2)$ to $O(|A|^3)$ for triplets,
but structured evaluation via FFT may reduce this. Early analysis suggests the entropy
density formulation from Paper 1 adapts to the $k = 3$ geometry with a modified curvature
parameter.

We present preliminary results at $n = 6$ with full census planned for $n = 7, 8$.
